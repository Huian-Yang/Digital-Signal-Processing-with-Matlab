...
%% Settings
SECONDS_TO_PLOT = 10;
ARDUINO_SERIAL_PORT = 'COM3'; % Change this (Win: 'COM?', MAC: '/dev/cu.usbmodem?')
...
axis_50hz=subplot(411);plot_50hz=plot(time_50hz,sample_50hz,'-o','linewidth',2);title('Photoresistor Sampled at 50Hz'),xlabel('Time(s)'),ylabel('Voltage');
axis_10hz=subplot(412);plot_10hz=plot(time_10hz,sample_10hz,'-o','linewidth',2);title('Photoresistor Sampled at 10Hz'),xlabel('Time(s)'),ylabel('Voltage');
axis_5hz=subplot(413);plot_5hz=plot(time_5hz,sample_5hz,'-o','linewidth',2);title('Photoresistor Sampled at 5Hz'),xlabel('Time(s)'),ylabel('Voltage');
axis_1hz=subplot(414);plot_1hz=plot(time_1hz,sample_10hz,'-o','linewidth',2);title('Photoresistor Sampled at 1Hz'),xlabel('Time(s)'),ylabel('Voltage');
