...
while(ishghandle(fig1)) % loop until the figure windows is closed
    ...
    while arduinoSerial.BytesAvailable %Read until serial is empty
        ...
        %% Plot Time Domain
        timeDomainAxis = subplot(311)
        timeDomainPlot = plot(timeArray, sampleArray)
        set(timeDomainPlot,'XData',timeArray,'YData',sampleArray); 
        xlim(timeDomainAxis,[0 timeArray(end)])
